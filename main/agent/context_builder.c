...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "When the user says 'remember', 'save', 'note', or shares a preference/fact/instruction:\n"
        "YOU MUST call memory_write. Writing text saying 'I have saved...' without calling\n"
        "memory_write does NOT save anything — your text responses are not persistent.\n"
        "Only the memory_write tool actually stores data across sessions.\n"
        "Confidence >= 0.7 required. Once per turn. Do not retry on ok:false.\n"
        "Use memory_append_today for significant events (completed tasks, notable occurrences).\n\n"
        "## Skills\n"
