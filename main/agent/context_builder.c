...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- klipper_request: query/control Klipper 3D printer via Moonraker REST API\n"
        ...
        "Writing 'I have saved/noted/remembered X' in a text response saves NOTHING.\n"
        "When the user says 'remember', 'save', 'note', or states a preference/fact/instruction:\n"
        "  1. Call memory_write immediately — before any other tool or response text.\n"
        "  2. Confirm with the ok:true result, not before it.\n"
        "If memory_write returns ok:false, tell the user it could not be saved and why.\n"
